...
class System_IndexController extends KD_Controller_Action {
			...
			{	
				foreach($ids as $key=>$id):
					...
					if($team)
					{
						...
						$this->flashMessenger->setNamespace('success')->addMessage($this->view->translate('User Moved Successfully to Team-'.$teamID));
					}
					else
					{
						$data['team_userID'] = $id;
	...
	public function createdepartmentpost($data) { 
		...
		else
		{
			...
			$this->flashMessenger->setNamespace('success')->addMessage('Department record was Successfully Added');
	...
	public function createuserpost($data) { 
        ...
        $this->flashMessenger->setNamespace('success')->addMessage('User record was Successfully Added');
        $this->_redirect('/user/index/');
	}
	public function createvaktraport($data,$patientID) { 
		...
		if($vaktrapId)
		{
			$this->flashMessenger->setNamespace('success')->addMessage('ScoreCard was Successfully Added');
...
//		die;
		...
		$this->flashMessenger->setNamespace('success')->addMessage('Client record was Successfully Added');
		$this->_redirect('/system/index');
	}	
	public function setConfig($data) { 
		KD::getModel('system/system')->update($data);
		$this->flashMessenger->setNamespace('success')->addMessage('System Config was Successfully Changed');
    ...
    public function eventsAction() {
        ...
        $department = null;
        if(isset($_SESSION['ACL']['userDeptId'][0]))  {
            $department = $_SESSION['ACL']['userDeptId'][0] === 'all' ? null :  $_SESSION['ACL']['userDeptId'][0];
        }
