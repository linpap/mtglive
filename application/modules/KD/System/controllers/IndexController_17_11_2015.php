...
class System_IndexController extends KD_Controller_Action {
			...
			{	
				foreach($ids as $key=>$id):
					...
					if($team)
					{
						...
						$this->flashMessenger->setNamespace('success')->addMessage($this->view->translate('User Moved Successfully to Team-'));
					}
					else
					{
						$data['team_userID'] = $id;
	...
	public function createdepartmentpost($data) { 
		...
		else
		{
			...
			$this->flashMessenger->setNamespace('success')->addMessage('Avdeling opprettet');
	...
	public function createuserpost($data) { 
        ...
        $this->flashMessenger->setNamespace('success')->addMessage('Behandler opprettet');
        $this->_redirect('/user/index/');
	}
	public function createvaktraport($data,$patientID) { 
		...
		if($vaktrapId)
		{
			$this->flashMessenger->setNamespace('success')->addMessage('Målstyringsplanen og vaktrapporten er nå opprettet for klienten');
	...
	public function createclientpost($data) { 
		...
		KD::getModel('client/client')->insert($data);
		$this->flashMessenger->setNamespace('success')->addMessage('Klient opprettet');
		$this->_redirect('/system/index');
	}	
	public function setConfig($data) { 
		KD::getModel('system/system')->update($data);
		$this->flashMessenger->setNamespace('success')->addMessage('Endringer er lagret');
