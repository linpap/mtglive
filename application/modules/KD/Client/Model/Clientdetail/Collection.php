 ...
 */
class KD_Client_Model_Clientdetail_Collection extends KD_Core_Model_Abstract
{
    ...
    protected function checkClientDetail($patient_id = 0)
    {
		$adapter = $this->_getResource()->getAdapter();
		$select = $adapter->select()->from('patient_detail')->where($adapter->quoteInto('patient_detail_patientID = ?', $patient_id));
		$stmt = $select->query();
