...
class Client_MtoController extends KD_Controller_Action {
		...
		if(isset($patientId) && $patientId>0 )
		{
			...
			foreach($_POST['tilfutDescs'] as $key => $tiltak)
			{
				if(isset($tiltak) && $tiltak!='')
				{
					...
					$data_inst= array('tilins_desc'=>$tiltak, 'tilins_owner'=>$_POST['tilfutowner'][$key], 'tilins_maalID'=>$_POST['maalIds'][$key],'tilins_patientID'=>$patientId,'from_date'=>$_POST['fromDates'][$key], 'to_date'=>$_POST['toDates'][$key],'tilins_vaktrapportID'=>$currentVaktrap,'tilins_previous_vaktrapportID'=>$previousVaktrap);
					//echo '<pre>';print_r($data);exit();
					//$_POST['maalDescs'][$key]
					KD::getModel('client/tiltakinst')->insert($data_inst);
    ...
    public function createShortTermGoalAction() {
		...
		$this->view->maalid = $this->getRequest()->getParam('maalid');
