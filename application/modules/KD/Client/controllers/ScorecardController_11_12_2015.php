    ...
    public function indexAction() {
       $clientID = $this->getRequest()->getParam('id');//exit();
		if(isset($clientID) && $clientID>0 )
		{
			...
			if(is_array($vaktrapportArray)){
				$vaktrapInfo= end($vaktrapportArray);
			}
			...
			$maalplanMaalAchiveInfo = KD::getModel('client/maalplanmaal')->loadAchiveList($clientID,$maalplanID = 0,$maal_achived_status='no');
			$this->view->maalplanMaalAchiveInfo = $maalplanMaalAchiveInfo;
//			echo "<pre>";
//			print_r($maalplanMaalAchiveInfo);
//			die;
			...
			if(isset($maalPlanID) && $maalPlanID>0)
			{
				...
				if($maalplanInfo['maalplan_patientID']==$clientID)
				{
					...
					foreach($maalplanMaalInfo as $maalplanMaal)
					{
						...
						$maalplanTiltakInfo[$maalplanMaal['maal_id']]['data'] = KD::getModel('client/maalplantiltak')->loadTiltakList($clientID,$maalPlanID,'archive');
					}
					$this->view->maalplanTiltakInfo = $maalplanTiltakInfo;
					$maalplanId = 0;
		...
		if(isset($clientID) && $clientID>0 && $clientIDPost==$clientID)
		{
			...
			if($maalPlanID==$maalPlanIDPost)
			{
				...
				if($maalPlanID>0)
				{
					...
					{
						//$obj1 = new Client_MtoController($this->_request, $this->_response);

						$archive=array('patient_id'=>$clientID);
						...
						{
							$archive['selAction']='http://localhost/mtg/zend/client/mto/archivemaal';
							foreach($_POST['maal'] as $maalId=>$maal)
							{
								if(isset($maal['maalplan_maalResult']) && $maal['maalplan_maalResult']==2){
									$archive['checkbox_'.$maalId]='on';
//									$maalPlanID = KD::getModel('client/maal')->archupdate($maalId,$clientID);
									$data = array('maal_status'=>'no');
									KD::getModel('client/maal')->update($data,'maal_id',$maalId);
								}
							}
							//$obj1->maalplanmaal($archive);
						...
						elseif(isset($_POST['maalplanmaal']) && count($_POST['maalplanmaal']) > 0)
						{
							...
							{
								if(isset($maalplanmaal['maalplan_maalResult']) && $maalplanmaal['maalplan_maalResult']==2){
									$archive['checkbox_'.$maalplanmaal['maal_id']]='on';
									$maalId=$maalplanmaal['maal_id'];
//									$maalPlanID = KD::getModel('client/maal')->archupdate($maalId,$clientID);
									$data = array('maal_status'=>'no');
									KD::getModel('client/maal')->update($data,'maal_id',$maalId);
								}
