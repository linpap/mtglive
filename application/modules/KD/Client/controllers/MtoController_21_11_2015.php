		...
		if(isset($patientId) && $patientId>0 )
		{
			...
			foreach($_POST['tilfutDescs'] as $key => $tiltak)
			{
				if(isset($tiltak) && $tiltak!='')
				{
					...
					$data_inst= array('tilins_desc'=>$tiltak, 'tilins_owner'=>$_POST['tilfutowner'][$key], 'tilins_maalID'=>$_POST['maalIds'][$key],'tilins_patientID'=>$patientId,'from_date'=>$_POST['fromDates'][$key], 'to_date'=>$_POST['toDates'][$key],'tilins_previous_vaktrapportID'=>0);
					//echo '<pre>';print_r($data);exit();
					//$_POST['maalDescs'][$key]
