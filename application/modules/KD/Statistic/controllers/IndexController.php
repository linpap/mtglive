...
class Statistic_IndexController extends KD_Controller_Action {
    ...
    public function departmentAction() {
		...
		// This code is for observation on for department so don't delete that
		...
		// This code is for observation on for department  ends so don't delete that 
		...
		if(isset($deptId) && $deptId>0 )
		{
			...
			$this->view->title = $this->view->translate('Statistikk For D').' '.$deptName.'';
			...
			if(isset($period, $year) && $period!='' && in_array($period,array('curyear','quartal1','quartal2','quartal3','quartal4','lifetime')) && isset($year) && $year>0)
			{
				...
				if(isset($startDate,$endDate))
				{
					...
					// Maal Chart Collection Starts
					$this->view->maalCountCollection = array();
					foreach($arrayResult as $key =>$maalRes)
					{
					  $dataRes = KD::getModel('client/maalplanmaal')->loadListByResult($deptId,$maalRes,$startDate,$endDate,true);
		...
		if(isset($clientID) && $clientID>0 )
		{
			...
			if(isset($period, $year) && $period!='' && in_array($period,array('curyear','quartal1','quartal2','quartal3','quartal4','lifetime')) && isset($year) && $year>0)
			{
				...
				else
				{
					switch($period)
					{
						...
						case 'lifetime':
							...
							$endDate = date('Y-m-d'); 
							$group = 'month';
							//$monthReports = KD::getModel('vaktrapport/vaktrapport')->getVaktrapMonths($clientID,$startDateObj->format('Y'));
						break;
					}
				}
				...
				if(isset($startDate,$endDate))
				{
					...
					if(is_array($vaktrapIds) && count($vaktrapIds)>0)
					{						
						// Government Chart Collection Starts
						//$tiltakGovResult = KD::getModel('vaktrapport/vaktraptilgov')->loadListByVaktrap($clientID,$vaktrapIds,false,true);
						$this->view->govTiltakCountCollection = array();
						foreach($arrayResult as $key =>$tiltakGov)
						{
						...
						// Government Chart Collection Ends
						
						// Institute Chart Collection Starts
						$this->view->instTiltakCountCollection = array();
						foreach($arrayResult as $key =>$tiltakInst)
						{
						...
						// Institute Chart Collection Ends
					}
					...
					foreach($observationAlls as $observationAll)
					{
						...
						{ 
							...
							$observationAllData[$tmpPeriod] = KD::getModel('vaktrapport/vaktrapobser')->loadListByObservationAllForReport($clientID,$startDate,$endDate,$group,$tmpPeriod);
								...
								if($i==0){
									$countingmax=count($observationAllData[$tmpPeriod]);
								}
								if($i==0){
									$countingmin=count($observationAllData[$tmpPeriod]);
								}
								if($countingmin > count($observationAllData[$tmpPeriod])){
									$countingmin=count($observationAllData[$tmpPeriod]);
								}

								if($countingmax <= count($observationAllData[$tmpPeriod])){
									$countingmax=count($observationAllData[$tmpPeriod]);
					   ...
					   $finalArray=array();
						foreach($observationAllData as $rowData){
							if(count($rowData) == $countingmax){
								$slicearray=array_slice($rowData,$countingmin);
							}
							if(count($rowData) < $countingmax){
								$sliceIndex=$countingmax-count($rowData);

								//$addArray=array_slice($slicearray,0,$sliceIndex);
								$addArray=Array
								(
									'period'=>0,
									'result' =>0 ,
								...
								for($x=0;$x<$sliceIndex;$x++){
									array_push($rowData,$addArray);
								}
								//$arr=array_merge($rowData,$addArray);
								array_push($finalArray,$rowData);
					...
					$this->view->observationAllData = $finalArray;
					$this->view->observations = $observations;
				}
				else
				{
