...
class User_IndexController extends KD_Controller_Action {
	...
	public function searchAction() {
		...
		if(isset($search) && $search!='')
		{
			...
			$userArray = KD::getModel('user/user')->loadUserBySearch($id="",$search,'user_code');

			$clientArray = KD::getModel('client/client')->loadClientBySearch($id="",$search,'patient_birk_no');

			$departmentInfoArray =  KD::getModel('department/department')->loadDeptBySearch($searchText);
...
//			die;
			if(is_array($userArray) && count($userArray)>0)
			{
				...
				$this->_redirect('/user/info/index/id/'.$userArray[0]['user_id']);
			}
			elseif(is_array($departmentInfoArray) && count($departmentInfoArray)>0)
			{
				//$this->_redirect('/client/info/index/id/'.$clientArray['patient_id']);
				$this->_redirect('/department/info/index/id/'.$departmentInfoArray[0]['dept_id']);

			}elseif(is_array($clientArray) && count($clientArray)>0)
			{
				 $this->_redirect('/client/info/index/id/'.$clientArray[0]['patient_id']);
				//$this->_redirect('/department/info/index/id/'.$departmentInfoArray[0]['dept_id']);
			}
			else
			{
				$this->flashMessenger->setNamespace('error')->addMessage($this->view->translate('You searched for "%s", but we found no user or department with this ID',$searchText));
