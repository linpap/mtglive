...
class KD_User_Model_User extends KD_User_Model_User_Collection
{
	...
	public function loadUserBySearch($id=0,$field=null, $_tableField=array())
	{
		...
		$select = $db->select()
			...
			->where($adapter->quoteInto('CONCAT(",",u.user_fname,",") LIKE ?', ','.$field.',') ." OR ". $adapter->quoteInto('CONCAT(",",u.user_mname,",") LIKE ?', ','.$field.',') ." OR ". $adapter->quoteInto('CONCAT(",",u.user_fname,",") LIKE ?', ','.$field.','))
