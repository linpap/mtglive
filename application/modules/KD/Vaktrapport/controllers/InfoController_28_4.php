		...
		if(isset($patientId) && ($patientId == $_POST['logg_patientID']))
		{
			...
			if(isset($vaktrapId) && ($vaktrapId == $_POST['logg_vaktrapID'])) 
			{
				...
				if(count($loggArray)>0 && ($loggArray['logg_id'] == $_POST['logg_id']))
				{
					...
					if($_POST['lock_logg'])
					{
						$data = array();$data['logg_status']='no';$data['logg_locked_by']=$_SESSION['Acl']['userID'];$data['logg_locked_at']=date("Y-m-d H:i:s");
						...
						parent::sendEmail('kedeesofttech@gmail.com','System','email.phtml',$dataEmail);
					}
				}
				else
				{
		...
		if(isset($patientId) && ($patientId == $_POST['force_patientID']))
		{
			...
			if(isset($vaktrapId) && ($vaktrapId == $_POST['force_vaktrapID'])) 
			{
				...
				{					
					...
					$force214Array = KD::getModel('client/force214')->getForceByVaktrap($vaktrapId,$patientId);
					// If Force No created for 2.1 - 2.4 Create New otherwise Update
					...
					$force2511Array = KD::getModel('client/force2511')->getForceByVaktrap($vaktrapId,$patientId);
					// If Force No created for 2.5 - 2.11 Create New otherwise Update
					...
					if($_POST['lock_force'])
					{
						$data['force_status']='no';$data['force_lock_by']=$_SESSION['Acl']['userID'];$data['force_lock_at']=date("Y-m-d H:i:s");
						$flag = KD::getModel('client/force')->update($data,'force_id',$_POST['force_id']);
				...
				else
				{
					...
					if($_POST['lock_force']){$data['force_status']='no';$data['force_locked_by']=$_SESSION['Acl']['userID'];$data['force_locked_at']=date("Y-m-d H:i:s");}
