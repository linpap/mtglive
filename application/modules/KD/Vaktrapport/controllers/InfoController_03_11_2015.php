			...
			if(isset($vaktrapID) && $vaktrapID > 0)
			{
				...
				if($vaktrapData['vaktrap_patientID'] == $patientID)
				{
					...
					if(!$errorFlag)
					{
                               ...
                               //$this->_logUserActionOnReport($vaktrapID, $patientID, 'Lock');
                                if($vaktrapLockFlag)
								{
									...
									if(isset($_POST['new_report']))
									{
                                        ...
                                        $this->_customelogUserActionOnReport($vaktrapInserId, $patientID, 'Create','M');
										if($vaktrapInserId)
										{
						...
						{
                            $this->_customelogUserActionOnReport($vaktrapID, $patientID, 'Save','M');
...
/**
	...
	protected function _customelogUserActionOnReport($reportId, $patientId, $action,$log_type) {
		$locked = ($action === 'Locked');
		$result = KD::getModel('client/logg')->insert(array(
			'logg_type' => $log_type,
			'logg_patientID' => $patientId,
