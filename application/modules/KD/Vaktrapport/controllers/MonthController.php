...
class Vaktrapport_MonthController extends KD_Controller_Action {
    ...
    private function _showReport($clientID, $method = 'POST', $reportType = 'maaned', $year = 0, $period = 0) {
        ...
        if ($reportType == 'maaned') {
            $redirectTab = '23';
            $reportName = $this->view->translate('Monthly');
        } else {
            $redirectTab = '24';
            $reportName = $this->view->translate('Quarterly');
        }
        ...
        if ((isset($clientID) && $clientID > 0)) {
            $this->view->title = $this->view->translate('%sSecurity Report For', $reportName) . '  ' . KD::getModel('client/client')->getClient($clientID, 'name') . '';
            $this->view->className = 'PTCLEFTVAKTRAPORT';
