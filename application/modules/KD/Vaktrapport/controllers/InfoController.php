...
use KD_Vaktrapport_Middleware_DeviationFacade as Deviation;
...
//			echo "<pre>";
//			print_r(unserialize($vaktrapArray[0]['vaktrap_counters']));
//			die;
//			echo "<pre>";
//			print_r($vaktrapArray);
//			die;
...
//				die;
				...
				$vaktrapInfo['vaktrap_tilspan_to_date'] = (isset($vaktrapInfo['vaktrap_tilspan_to_date'])
					...
					: $maalplanInfo['maalplan_from_date'];
				...
				if(count($deviationArray)>0)$deviationArray=$deviationArray[0];
				$deviationArray['vaktrapId']=$vaktrapID;
				$deviationArray['patientId'] = $patientID;
				$this->view->deviationInfo = $deviationArray;
                ...
                $this->view->treatmentPersonnel = TreatmentPersonnel::getTreatmentPersonnele($vaktrapID, $patientID, $vaktrapInfo);
...
//			die;
			...
			if(!empty($vaktrapArray) && $vaktrapArray[0]['vaktrap_status']=='no')
			{
				...
				$this->view->logArchive = KD::getModel('client/logg')->getLoggByVaktrap($vaktrapID, $patientID, null);
				$this->view->deviationArchive = KD::getModel('client/deviation')->getDeviationByVaktrap($vaktrapID, $patientID, null);
				$this->view->className = 'PTCLEFTVAKTRAPORT';
				...
				if($vaktrapData['vaktrap_patientID'] == $patientID)
				{
					...
					if(!empty($vakGovTiltakRes))
					{
						foreach($vakGovTiltakRes as $vakGovTiltakId=> $vakGovTiltakResult)
						{
							...
							if(isset($vakGovTiltakExpl[$vakGovTiltakId]) && ($vakGovTiltakResult!=2)){
								$data['vaktrap_tilgov_explanation'] = $vakGovTiltakExpl[$vakGovTiltakId];
							}else{
								$data['vaktrap_tilgov_explanation'] = '';
							}
...
//					die;
					if(!empty($vakInsTiltakRes))
					{
						foreach($vakInsTiltakRes as $vakInsTiltakId=> $vakInsTiltakResult)
						{
							...
							if(isset($vakInsTiltakExpl[$vakInsTiltakId]) && ($vakInsTiltakResult!=2)){
								$data['tilins_explanation'] = $vakInsTiltakExpl[$vakInsTiltakId];
							}else{
								$data['tilins_explanation'] = '';
							}
					...
					$deviationArray = KD::getModel('client/deviation')->getDeviationByVaktrap($vaktrapID,$patientID,false);
					// Force Collection
...
//						}
					...
					$vaktrapCounters['avvik'] = count($deviationArray);
...
/**
	...
	public function savedeviationAction()
	{
		...
		if(!Deviation::save($patientId, $this->getRequest())) {
			return $this->_httpErrorRedirect(Deviation::getResponseText(), '/vaktrapport/index/index');
		}
		
		return $this->_httpSuccessRedirect(
			Deviation::getResponseText(),
			'/vaktrapport/info/index/t/6/id/' . Deviation::getReportId()
		);
