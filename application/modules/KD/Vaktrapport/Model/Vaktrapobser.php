...
class KD_Vaktrapport_Model_Vaktrapobser extends KD_Vaktrapport_Model_Vaktrapobser_Collection
{
	...
	//
	public function loadListByVaktrap($patientId = 0, $startDate = '', $endDate = '',  $type='active', $isDept=false)
	{
		if($patientId > 0)
		{
			$db = $this->_getResource();
			...
			if($isDept)
			{
				$select = $select->join(array('p'=>'patient'),'vo.vaktrap_patientID=p.patient_id',array('patient_deptID'))->where($adapter->quoteInto('p.patient_deptID = ?', $patientId));
			}
			else
			{
			...
			$select = 	$select->where($adapter->quoteInto('vo.vaktrap_obser_date >= ?', $startDate))
	...
	// Get List of all Vaktrapport Observation result according to Patient, Vaktrapport ID and Observation ID 
	public function loadListByObservationForReport($patientId = 0, $startDate = '', $endDate = '', $obserId=0, $group='date', $checked=false, $isDept=false)
	{
		if($patientId > 0 && $obserId > 0)
		{
			$db = $this->_getResource();
			...
			if($isDept)
			{
				$select = $select->join(array('p'=>'patient'),'vo.vaktrap_patientID=p.patient_id',array('patient_deptID'))
							->where($adapter->quoteInto('p.patient_deptID = ?', $patientId));
			}
			else
			{
				$select = $select->where($adapter->quoteInto('vo.vaktrap_patientID = ?', $patientId));
	...
	// Get List of all Vaktrapport Observation result according to Patient, Vaktrapport ID and Observation ID 
	public function loadListByObservationAllForReport($patientId = 0, $startDate = '', $endDate = '', $group='date', $having='', $isDept=false)
	{
		if($patientId > 0)
		{
			$db = $this->_getResource();
			...
			{
				$select = $select
							->join(array('p'=>'patient'),'vo.vaktrap_patientID=p.patient_id',array('patient_deptID'))
							->where($adapter->quoteInto('p.patient_deptID = ?', $patientId));
			}
			else
			{
